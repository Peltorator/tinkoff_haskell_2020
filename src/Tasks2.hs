...
n'th_prime :: Int -> Int
n'th_prime = (primes !!)
    where primes = [x | x <- [2..], null [d | d <- [2..(floor (sqrt (fromIntegral x)))], x `mod` d == 0]]
...
tails' :: [a] -> [[a]]
tails' = foldr updateTails [[]]
    where updateTails val list = (val:head list):list
...
-- Необходимо реализовать функцию при помощи foldr.
inits' :: [a] -> [[a]]
inits' = foldr updateInits [[]]
    where updateInits val list = ([] : (map ((:) val) list))

-- 15. reverse' переворачивает список, который был дан на входе.
-- Необходимо реализовать функцию при помощи foldr.
reverse' :: [a] -> [a]
reverse' = foldr pushBack []
    where pushBack val list = list ++ [val]
